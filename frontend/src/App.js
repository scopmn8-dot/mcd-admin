...
const drawerWidth = 260; // Optimized for responsive design
...
function App() {
  ...
  const navigationItems = [
    ...
    // Job Management
    {
      ...
      items: [
        { label: 'Processed Jobs', icon: <JobsIcon />, index: 3, description: 'View completed job history and manage driver assignments' },
    ...
    <ThemeProvider theme={theme}>
      ...
      <Box sx={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
        ...
        >
          <Toolbar sx={{ 
            ...
            <Stack direction="row" alignItems="center" spacing={{ xs: 0.5, sm: 1 }} sx={{ flexShrink: 0 }}>
              ...
              <Tooltip title={autoRunEnabled ? 'Auto-run is enabled' : 'Enable auto-run (every 5min)'} arrow>
                <span>
                  <IconButton
              ...
              {/* Auto Runner Status - Only show on desktop */}
              {!isMobile && !isTablet && (
                <Paper 
                  elevation={0} 
                  sx={{ 
                ...
                >
                  <Box sx={{ display: 'flex', alignItems: 'center', width: '100%', minWidth: 0 }}>
                    ...
                    <Box sx={{ ml: 1, display: 'flex', gap: 0.5, flexDirection: 'column', flexShrink: 0 }}>
                      <Box sx={{ display: 'flex', gap: 0.5, flexWrap: 'wrap' }}>
                        <Chip 
                          label="Clstr" 
                          size="small" 
                          color={autoRunSteps.clustering === 'running' ? 'info' : autoRunSteps.clustering === 'success' ? 'success' : autoRunSteps.clustering === 'error' ? 'error' : 'default'} 
                          sx={{ fontSize: '0.6rem', height: 18 }}
                        />
                        <Chip 
                          label="Assgn" 
                          size="small" 
                          color={autoRunSteps.assignDrivers === 'running' ? 'info' : autoRunSteps.assignDrivers === 'success' ? 'success' : autoRunSteps.assignDrivers === 'error' ? 'error' : 'default'} 
                          sx={{ fontSize: '0.6rem', height: 18 }}
                        />
                        <Chip 
                          label="IDs" 
                          size="small" 
                          color={autoRunSteps.assignIds === 'running' ? 'info' : autoRunSteps.assignIds === 'success' ? 'success' : autoRunSteps.assignIds === 'error' ? 'error' : 'default'} 
                          sx={{ fontSize: '0.6rem', height: 18 }}
                        />
                        <Chip 
                          label="Cmbd" 
                          size="small" 
                          color={autoRunSteps.combinedJobs === 'running' ? 'info' : autoRunSteps.combinedJobs === 'success' ? 'success' : autoRunSteps.combinedJobs === 'error' ? 'error' : 'default'} 
                          sx={{ fontSize: '0.6rem', height: 18 }}
                        />
                      </Box>
                      <Button 
              ...
              {/* Compact Auto Runner Status for tablet */}
              {isTablet && !isMobile && (
                <Paper 
                  elevation={0} 
                  sx={{ 
                ...
                >
                  <Chip 
                    label={autoRunEnabled ? 'Auto ON' : 'Auto OFF'} 
                    size="small" 
                    color={autoRunEnabled ? 'success' : 'default'}
        ...
        {isAuthenticated && (
          ...
          >
          <Toolbar sx={{ minHeight: { xs: '64px !important', sm: '72px !important' } }} /> {/* Spacer for app bar */}
